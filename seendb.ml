...
object
  ...
  method! enter _path _chunk node = match node with
  ...
  method! leave _path chunk node = match node with
...
let remove_expired nodes =
  Int64Map.filterv (fun { n_expire; _ } -> n_expire > start_time) nodes
...
let get cache pino =
  match Db.sqln cache "select info from seen where pino = ?" [Db.Data.INT pino] with
    ...
    | [ [| Db.Data.BLOB data |] ] ->
      ...
      let as_pair ({ n_inode; _ } as node) = (n_inode, node) in
